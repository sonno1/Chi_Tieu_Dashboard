...
function buildExportWorkbook(
  ...
  // Amount number format (comma style, 0 decimals)
  const amountColIndex = headers.indexOf("Số tiền"); // 0-based
  ...
  // Format numeric values in summary (except count)
  for (let r = 1; r <= 4; r++) {
    const addr = XLSX.utils.encode_cell({ r, c: 1 }); // value column
